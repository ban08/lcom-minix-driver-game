...
/// @brief Mouse IRQ hook ID (default 5)
int m_h_id = 5;

/// @brief Mouse packet structure used to store the parsed packet
struct packet m_pckt;

/// @brief Raw bytes from mouse controller
uint8_t mouse_ctrler_bytes[3];

/// @brief Most recently read byte from mouse
uint8_t curr_byte;

/// @brief Index to track current byte in packet
uint8_t byte_idx = 0;
...
int (mouse_write)(uint8_t command)
{
  ...
  do {
    ...
    if (writeKBControllercommand(KBC_IN_COMMAND, WR_BYTE_M) ||
        writeKBControllercommand(KBC_OUT_COMMAND, command))
    {
      return 1;
    }
...
void m_sync_pckt()
{
  // Accept only valid first byte or middle of packet
